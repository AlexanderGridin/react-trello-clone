...
export const initBoardsMock = (adapter: MockAdapter) => {
  ...
  adapter.onPost(routes.addBoard).reply((config: any) => {
    const board = {
      _id: generateId(),
  ...
  adapter.onGet(urlRegExp).reply((config: any) => {
  ...
  adapter.onPut(urlRegExp).reply((config: any) => {
    const id = config.url?.split("/")?.[2] ?? generateId();
  ...
  adapter.onDelete(urlRegExp).reply((config: any) => {
    const id = config.url?.split("/")?.[2] ?? generateId();
    const board = boardsDb.getById(id);
