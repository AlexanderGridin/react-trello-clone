...
import { routes } from "../../TasksList/routes";
...
export const initTasksListMock = (adapter: MockAdapter) => {
  adapter.onPost(routes.addTasksList).reply((config: any) => {
  ...
  adapter.onPut(/\/tasks-list\//).reply((config: any) => {
  ...
  adapter.onDelete(/\/tasks-list\//).reply((config: any) => {
    const id = config.url?.split("/")?.[2] ?? generateId();
    const list = {
      ...tasksLists.getById(id),
    };
